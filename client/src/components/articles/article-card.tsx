...
export default function ArticleCard({ article, showLockIcon = true, featured = false }: ArticleCardProps) {
  const formatDate = (date: Date) => {
    return formatDistanceToNow(new Date(date), { addSuffix: true });
  };

  // If featured is true, render a horizontal card with larger image
  if (featured) {
    return (
      <Card className="bg-neutral-50 overflow-hidden shadow-md transition-all hover:shadow-lg">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 p-6">
            ...
            <Link href={`/articles/${article.id}`}>
              <h3 className="font-bold text-2xl mb-3 font-poppins hover:text-primary cursor-pointer">
                {article.title}
              </h3>
            </Link>
