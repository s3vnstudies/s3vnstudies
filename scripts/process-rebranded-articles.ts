...
async function main() {
  try {
    ...
    for (const mdFile of mdFiles) {
      ...
      const existingArticles = await db.select().from(articles).where(eq(articles.title, data.title as string));
      ...
      if (existingArticles.length > 0) {
        ...
        await db.update(articles)
          .set({
            ...
            author: data.author as string || 'S3VN Studies Editorial Team',
            thumbnail: data.thumbnail as string || (imageFile ? `/images/articles/self-help/${imageFile}` : null),
            images: imageFile ? [`/images/articles/self-help/${imageFile}`] : [],
            excerpt,
            category,
            membershipRequired
          })
          .where(eq(articles.title, data.title as string));
      } else {
